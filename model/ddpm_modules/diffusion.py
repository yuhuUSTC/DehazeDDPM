...
class GaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def p_sample_loop(self, condition, J, transmission):
        # x_start = J
        ...
        if not self.conditional:
            ...
            for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
                img = self.p_sample(img, i, condition_x=x)
                # if i == t:
                #     predicted_image = torch.clamp((x_noisy + 1.0) / 2.0, min=0.0, max=1.0)
                #     inpainted = predicted_image.cpu().numpy().transpose(0, 2, 3, 1)[0] * 255
                #     Image.fromarray(inpainted.astype(np.uint8)).save(r"D:\1.png")
                #
                #     predicted_image = torch.clamp((img + 1.0) / 2.0, min=0.0, max=1.0)
                #     inpainted = predicted_image.cpu().numpy().transpose(0, 2, 3, 1)[0] * 255
                #     Image.fromarray(inpainted.astype(np.uint8)).save(r"D:\2.png")
                #
                #     img = x_noisy * transmission + img * (1 - transmission)
                #
                #     predicted_image = torch.clamp((img + 1.0) / 2.0, min=0.0, max=1.0)
                #     inpainted = predicted_image.cpu().numpy().transpose(0, 2, 3, 1)[0] * 255
                #     Image.fromarray(inpainted.astype(np.uint8)).save(r"D:\3.png")
    ...
    @torch.no_grad()
    def super_resolution(self, condition, J, t):
        return self.p_sample_loop(condition, J, t)
    ...
    def p_losses(self, x_in, condition, noise=None):
        ...
        if not self.conditional:
            ...
            #x_recon = self.denoise_fn(torch.cat([x_in['SR'], x_in['Density'], x_in['Structure'], x_noisy], dim=1), continuous_sqrt_alpha_cumprod)
...
#         return posterior_mean, posterior_log_variance_clipped
#
#     def p_mean_variance(self, x, x_in, t, clip_denoised: bool, condition_x=None):
#         batch_size = x.shape[0]
#         noise_level = torch.FloatTensor([self.sqrt_alphas_cumprod_prev[t+1]]).repeat(batch_size, 1).to(x.device)
#         if condition_x is not None:
#             x_recon = self.predict_start_from_noise(x, t=t, noise=self.denoise_fn(torch.cat([condition_x, x_in['Density'], x_in['Structure'], x], dim=1), noise_level))
#         else:
#             x_recon = self.predict_start_from_noise(x, t=t, noise=self.denoise_fn(x, noise_level))
#
#         if clip_denoised:
...
#
#     @torch.no_grad()
#     def p_sample(self, x, x_in, t, clip_denoised=True, condition_x=None):
#         model_mean, model_log_variance = self.p_mean_variance(x=x, x_in=x_in, t=t, clip_denoised=clip_denoised, condition_x=condition_x)
#         noise = torch.randn_like(x) if t > 0 else torch.zeros_like(x)
#         return model_mean + noise * (0.5 * model_log_variance).exp()
#
#     @torch.no_grad()
...
#             ret_img = x
#             for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
#                 img = self.p_sample(img, x_in, i, condition_x=x)
#                 if i % sample_inter == 0:
#                     ret_img = torch.cat([ret_img, img], dim=0)
#         if continous:
#             return ret_img
