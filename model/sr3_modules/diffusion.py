...
class GaussianDiffusion(nn.Module):
    ...
    def p_losses(self, x_in, condition, noise=None):
        ...
        x0_recon = self.predict_image(x_noisy=x_noisy, continuous_sqrt_alpha_cumprod=continuous_sqrt_alpha_cumprod.view(-1, 1, 1, 1), noise=x_recon)
        x0_recon = torch.abs(torch.fft.fftn(x0_recon, dim=(-2, -1)))
        x_in = torch.abs(torch.fft.fftn(x_in, dim=(-2, -1)))
        loss_frequency = self.loss_func(x0_recon, x_in)
        
        return loss + 0.01 * loss_frequency
